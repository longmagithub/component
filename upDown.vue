<template>
  <div ref="container" class="vue-pull-refresh">
    ...
    <div class="vue-pull-loadmore-msg"  ref='loadmore'>
      <template v-if="loading">
        <svg t="1497367491334"
          viewBox="0 0 1024 1024"
          class="vue-pull-refresh-loading"
  ...
  export default {
    ...
    data() {
      return {
        msg: '',
        flag: 0, // 表示是否达到刷新条件
        loading: 0,  // 表示是否下拉方法正在刷新中
        touchStart: 0,  // 手指触摸屏幕的起点
        distance: 0,     // 手指向下滑动的距离
        upDistance: 0,   // 手指向上滑动距离
        uploading: 0,    //  表示是否下拉方法正在刷新中
        loadmoreStyle: {}
      }
    },
    mounted() {
      // console.log(this.$slots.content[0].elm.clientHeight)
      // this.loadmoreStyle = {
      //   // top: this.$refs.content.clientHeight
      //   top: this.$slots.content[0].elm.clientHeight
      // }
      ...
      container.addEventListener('touchmove', (e) => {
        ...
        if (scrollTop === 0 && this.canUp) {
          ...
          if (this.distance > 0) {
            ...
            if (this.distance < MAX_DISTANCE) {
              ...
              // container.style.transform = 'translate3D(0px, ' + this.distance > MAX_DISTANCE ? MAX_DISTANCE : this.distance + 'px, 0px)'
              if (this.distance > REFRESH_DISTANCE) {
                  this.msg = '释放刷新'
                  this.flag = 1
              } else {
                  this.msg = '下拉刷新'
              ...
              container.style.overflow = 'visible'
              container.style.transform = 'translate3D(0px, ' + MAX_DISTANCE + 'px, 0px)'
              this.msg = '释放刷新'
              this.flag = 1
            }
          }
        ...
        if (scrollBottom === 0 && this.canDown) {
              // console.log(this.upDistance)
          this.upDistance = this.touchStart - touch.clientY
          if (this.upDistance > 0) {
            e.preventDefault()
            if (this.upDistance < MAX_DISTANCE) {
              // container.style.overflow = 'visible'
              container.style.transform = 'translate3D(0px, -' + this.upDistance + 'px, 0px)'
              if (this.upDistance > REFRESH_DISTANCE) {
                this.msg = '释放加载更多'
                this.flag = 1
              } else {
                this.msg = '上拉加载更多'
      ...
      container.addEventListener('touchend', (e) => {
        ...
        if (this.flag && this.distance > REFRESH_DISTANCE) {
        ...
        if (this.flag && this.upDistance > REFRESH_DISTANCE) {
          container.style.transform = 'translate3D(0px, -50px, 0px)'
          this.loading = 1
          this.down().then(() => {
            this.flag = 0
...
<style scoped>
  ...
  .vue-pull-loadmore-msg .icon-reverse{
